...
import { setViewNotes } from "../../redux/slices/noteSlice";
...
const SelectClassMarksScreen = () => {
  ...
  const handleSelect = async (item) => {
    dispatch(setViewNotes({ name: "selectedClass", value: item }));
    try {
      const classId = item._id;
      const subjectResult = await dispatch(getSubjects({ classId }));
      if (getSubjects.fulfilled.match(subjectResult)) {
    ...
    >
      <View style={styles.card}>
        ...
        <Text style={styles.cardText}>
          {item.name} - {item.section} ({item.timeDuration?.name})
        </Text>
        <Ionicons
  ...
  return (
    ...
    >
      <View style={styles.container}>
        <PageHeader text="Select Class for Notes" />
